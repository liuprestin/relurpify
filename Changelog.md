...
Shell setup now records whether your Ollama model supports tool-calling; the prompt defaults to disabled so models without function-call support (like deepseek-r1:7b) work out of the box. When tool-calling is off, relurpify switches to a new manual coding agent that asks the model for explicit JSON/markdown edit instructions and applies them itself, so generated files actually appear even without function calls. The agent now tolerates models that emit fenced code blocks, loose bullet lists, or trailing commas. The toggle lives in `.relurpify/config.json` and other commands honor it automatically.
